...
class OrderController extends Controller
{
    public function list(){
        //hóa đơn
        $orders = Order::query()->latest()->first();
        $orderItems = OrderItem::query()->where('order_id', $orders->id)->get();
        $totalAmount = 0;
        foreach (collect($orderItems) as $item) {
            $totalAmount += $item['quantity'] * ($item['product_price_sale'] ?: $item['product_price']);
        }
        return view('order', compact('orders','orderItems', 'totalAmount'));
    }
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('order.list')->with('success', 'Đặt hàng thành công');
